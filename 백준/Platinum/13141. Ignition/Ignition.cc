...
/*
  X 정점에서 불이 붙기 시작했을 때 A-B 간선이 타는 시간을 구하기 위해서는 아래
  3가지가 필요하다.
  1. X -> A 정점으로 불이 이동하는 최단시간
  2. X -> B 정점으로 불이 이동하는 최단시간
  3. A-B 간선의 길이

  (1+2+3)/2 로 해당 간선이 불에 타는 최소시간을 구할 수 있다. (이해가 안갈 경우
  X-A-B를 연결한 고무줄을 반으로 자른다고 생각해보자.) 모든 간선이 불에 타는
  시간은 가장 긴 간선이 불에 타는 시간과 동일하다.

  1과 2를 구하기 위해 플로이드-워셜을 이용한다.
  어떤 정점에서 시작했을 때 가장 빠르게 그래프가 불에 타는지 알 수 없기 때문에
  브루트포스로 답을 구한다.
*/